...
def get_VAR():
    Nei = []
    for I in G.nodes:
        Nei.append(len([n for n in G[I] if NowStrategiesList[n] != -1]))
    return np.var(Nei)
...
VARList = []
for indiv in G.nodes:
    VARList.append(len([n for n in G[indiv]]))
VAR_0 = np.var(VARList)
    ...
    for p in Rhos:
        ...
        while NowTime < IteratedTime:
            ...
            if current_time > last_recorded_time:
                ...
                result_V.append(get_VAR())
        plot_V.append(sum(result_V[-200:]) / len(result_V[-200:]))
        plot_C.append(sum(result_C[-200:]) / len(result_C[-200:]))
heatmap_V.append(plot_V)
...
plt.plot(Rhos, plot_V, label=r'Numerial $\sigma^2(\rho)$', color='#2D85F0', linewidth=5)
plt.plot(Rhos, VAR_0 * Rhos**2 + Rhos * (1 - Rhos) * (2 * G.number_of_edges() / G.number_of_nodes()) , label=r'Theoretical $\sigma^2(\rho)$', color='#F4433C', linestyle="--", linewidth=5)

rho_max_index = np.argmax(plot_V)
rho_max = Rhos[rho_max_index]
theoretical_vals = VAR_0 * Rhos**2 + Rhos * (1 - Rhos) * (2 * G.number_of_edges() / G.number_of_nodes())
rho_max_theoretical = Rhos[np.argmax(theoretical_vals)]

plt.axvline(x=rho_max, color='#2D85F0', label=r'Numerial Max', linestyle='--', linewidth=3)
plt.axvline(x=rho_max_theoretical, color='#F4433C', label=r'Theoretical Max' ,linestyle=':', linewidth=3)

plt.xlabel(r'$\rho$')
plt.ylabel(r'$\sigma^2(\rho)$')
