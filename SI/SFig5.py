    ...
    if len(SelectNeighbors) == 0:
        if NowStrategiesList[UpdateNode] == -1:
            StrategiesQueueList[UpdateNode].enqueue(
                Strategy((StrategiesQueueList[UpdateNode].lastStrategies).theStrategy,
                         random.expovariate(MuList[UpdateNode]), EnqueueTime=NowTime))
            return
        else:
            StrategiesQueueList[UpdateNode].enqueue(
...
def StrategiesQueueListInit(StrategiesQueueList, NowTime):
    ...
    half = total_queues // 10

    selected_indices = random.sample(range(total_queues), half)
    for index in selected_indices:
        StrategiesQueueList[index].enqueue(Strategy(1, random.expovariate(MuList[index]), EnqueueTime=NowTime))

    for index, queue in enumerate(StrategiesQueueList):
        if index not in selected_indices:
            queue.enqueue(Strategy(0, random.expovariate(MuList[index]), EnqueueTime=NowTime))
...
for i in range(AverageNum):
    ...
    while NowTime < IteratedTime:
        ...
        if current_time > last_recorded_time:
            ...
            satisfy.append(test_C_critical())
...
s = satisfy.astype(int)
cut_indices = np.where(np.diff(s) != 0)[0] + 1
bounds = np.r_[0, cut_indices, len(s)]

for start, end in zip(bounds[:-1], bounds[1:]):
    color = '#2D85F0' if satisfy[start] else '#F4433C'
    left = times[start]
    right = times[end] if end < len(times) else times[-1]
    plt.axvspan(left, right, facecolor=color, alpha=0.3, zorder=1)
