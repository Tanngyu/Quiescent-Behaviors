...
def ImitateUpdate(G, UpdateNode, NowTime):
    ...
    if len(SelectNeighbors) == 0:
        if NowStrategiesList[UpdateNode] == -1:
            StrategiesQueueList[UpdateNode].enqueue(
                Strategy((StrategiesQueueList[UpdateNode].lastStrategies).theStrategy, 1/MuList[UpdateNode], EnqueueTime=NowTime))
            return
        else:
            StrategiesQueueList[UpdateNode].enqueue(
                Strategy(NowStrategiesList[UpdateNode].theStrategy, 1/MuList[UpdateNode], EnqueueTime=NowTime))
            return
    ...
    if random.random() <= ((NeighborPayoff - SelfPayoff)/(b*Kd)):
        StrategiesQueueList[UpdateNode].enqueue(
            Strategy(NowStrategiesList[Neighbor].theStrategy, 1/MuList[UpdateNode], EnqueueTime=NowTime))
        return
    else:
        if NowStrategiesList[UpdateNode] == -1:
            StrategiesQueueList[UpdateNode].enqueue(
...
def StrategiesQueueListInit(StrategiesQueueList, NowTime):
    ...
    for index in selected_indices:
        StrategiesQueueList[index].enqueue(Strategy(0, 1/MuList[index], EnqueueTime=NowTime))
    for index, queue in enumerate(StrategiesQueueList):
        if index not in selected_indices:
            queue.enqueue(Strategy(1, 1/MuList[index], EnqueueTime=NowTime))
...
for p in Rhos:
    ...
    plot_D = []
    plot_L = []
    for IndividualNum in Individual_Nums:
        sigma = 0.1
        G = nx.watts_strogatz_graph(IndividualNum, AverageDegree, 0.2, 42)
        result_C = []
        times = []
        ...
        while NowTime < IteratedTime:
            ...
            if current_time > last_recorded_time:
                if current_time % 100 == 0:
                    print(f"Current time: {p}/{IndividualNum}/{NowTime}/{IteratedTime}")
...
with open('D.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['rho/sigma'] + Rhos)
    transposed = list(zip(*heatmap_C))
    for i, sigma in enumerate(Individual_Nums):
