...
MuList = [1.0 for _ in range(IndividualNum)]
...
def ImitateUpdate(G, UpdateNode, NowTime):
    ...
    if len(SelectNeighbors) == 0:
        if NowStrategiesList[UpdateNode] == -1:
            ...
            StrategiesQueueList[UpdateNode].enqueue(
                Strategy(NowStrategiesList[UpdateNode].theStrategy,
                random.expovariate(MuList[UpdateNode]), EnqueueTime=NowTime))
            return
    ...
    if random.random() <= probability:
        StrategiesQueueList[UpdateNode].enqueue(
            Strategy(NowStrategiesList[Neighbor].theStrategy,
            random.expovariate(MuList[UpdateNode]), EnqueueTime=NowTime))
        return
    else:
        if NowStrategiesList[UpdateNode] == -1:
...
def StrategiesQueueListInit(StrategiesQueueList, NowTime):
    ...
    for index in selected_indices:
        StrategiesQueueList[index].enqueue(Strategy(1, 1/MuList[index] * 0.5, EnqueueTime=NowTime))
    for index, queue in enumerate(StrategiesQueueList):
        if index not in selected_indices:
            queue.enqueue(Strategy(0, 1/MuList[index] * 0.5, EnqueueTime=NowTime))
...
plt.plot(times, result_C, label=r'$C$', color='#036EB8', linewidth=5)
plt.plot(times, result_D, label=r'$D$', color='#E60012', linewidth=5)
