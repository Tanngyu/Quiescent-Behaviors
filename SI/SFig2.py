    ...
    def dequeue(self, NowTime):
        if self.front == -1:
            ...
            if self.front == self.rear:
                ...
                size = self.size - self.front + self.rear + 1

            k = random.randint(0, size - 1)
            idx = (self.front + k) % self.size
            item = self.queue[idx]
            item.setUsedTime(NowTime)
            self.lastStrategies = item

            temp = [self.queue[(self.front + i) % self.size] for i in range(size) if i != k]
            ...
            if size == 1:
                ...
                for i in range(size - 1):
                    self.queue[i] = temp[i]
                for i in range(size - 1, self.size):
                    self.queue[i] = -1
            return item
...
with open('heatmap_C_R.csv', 'w', newline='') as csvfile:
...
with open('heatmap_D_R.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['b/rho'] + Rhos)
    for i, b in enumerate(b_s):
        row = [b] + heatmap_D[i]
...
with open('heatmap_L_R.csv', 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['b/rho'] + Rhos)
    for i, b in enumerate(b_s):
        row = [b] + heatmap_L[i]
